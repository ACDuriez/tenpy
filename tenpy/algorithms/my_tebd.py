...
class TEBDEngine(TimeEvolutionAlgorithm):
    ...
    """
    def __init__(self, psi, model, options,clifford=None, clifford_step=10, **kwargs):
        ...
        self.clifford = clifford if clifford is not None else None
        self.clifford_step = clifford_step
    ...
    def prepare_evolve(self, dt):
        ...
        if self.evolved_time != 0 and self.clifford is not None and (int(self.evolved_time/self.options['dt']) % self.clifford_step == 0 or int(self.evolved_time/self.options['dt']) % self.clifford_step == 1):
    ...
    def _calc_U_bond(self, i_bond, dt, type_evo, E_offset,apply_clifford=False):
        ...
        * ``U_bond = exp(- dt H_bond)`` for ``type_evo='imag'``.
        ...
        time_evolution_op = npc.expm(H2)
        if self.clifford is not None and int(self.evolved_time/self.options['dt']) % self.clifford_step == 0 and self.evolved_time!=0:
            clifford = time_evolution_op.copy(deep=True)
            clifford._data = [self.clifford]
            U = npc.tensordot(time_evolution_op,clifford,axes=1)
        else:
            U = npc.expm(H2)
